...
# (see bindings/libkrun-via-cdylib-weak/build.rs). Run this after changing the
# libkrun ffier API.
gen-libkrun-bindings:
	cargo build -p libkrun --features ffi $(FEATURE_FLAGS)
	cp target/ffier-krun.json bindings/libkrun-via-cdylib-weak/ffier-krun.json
...
# The Rust client is generated at compile time from the committed JSON
# (see bindings/init-blob-via-cdylib/build.rs). Run this after changing the
# init-blob ffier API.
gen-init-blob-bindings:
	cargo build -p krun-init-blob --features ffi
	cp target/ffier-krun_init.json bindings/init-blob-via-cdylib/ffier-krun_init.json

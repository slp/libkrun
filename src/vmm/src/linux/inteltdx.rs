...
impl IntelTdx {
    ...
    pub fn tdx_capabilities(&self, kvm: &Kvm) -> Result<TdxCapabilities, Error> {
        ...
        let mut cmd = kvm_tdx_cmd {
            id: kvm_bindings::kvm_tdx_cmd_id_KVM_TDX_CAPABILITIES,
            metadata: 0,
    ...
    fn tdx_init(&self, vm_fd: &VmFd, cpuid: &CpuId) -> Result<(), Error> {
        ...
        let mut cmd = kvm_tdx_cmd {
            id: kvm_bindings::kvm_tdx_cmd_id_KVM_TDX_INIT_VM,
            metadata: 0,
    ...
    fn tdx_init_region(
        ...
        let mut cmd = kvm_tdx_cmd {
            id: kvm_bindings::kvm_tdx_cmd_id_KVM_TDX_INIT_MEM_REGION,
            metadata: 1,
            data: &data as *const _ as u64,
        };
    ...
    fn tdx_finalize(&self, vm_fd: &VmFd) -> Result<(), Error> {
        let mut cmd = kvm_tdx_cmd {
            id: kvm_bindings::kvm_tdx_cmd_id_KVM_TDX_FINALIZE_VM,
            metadata: 0,
            data: 0,
        };
    ...
    pub fn init_vcpu(&self, vcpu_fd: &VcpuFd) -> Result<(), Error> {
        let mut cmd = kvm_tdx_cmd {
            id: kvm_bindings::kvm_tdx_cmd_id_KVM_TDX_INIT_VCPU,
            metadata: 0,
            data: 0,
        };

...
pub struct Apic {
    ...
    /// Entries
    entries: HashMap<u32, kvm_bindings::kvm_irq_routing_entry>,
}
    ...
    fn ioapic_write(&mut self, val: u32) {
        match self.ioregsel {
            ...
            _ => {
                ...
                if msi_data != 0 {
                self.entries.entry(index as u32).or_insert(kvm_bindings::kvm_irq_routing_entry {
                         gsi: index as u32,
                         ...
                         u: kvm_bindings::kvm_irq_routing_entry__bindgen_ty_1 {
                             msi: kvm_bindings::kvm_irq_routing_msi {
                                 ...
                                 data: msi_data as u32,
                                 ..Default::default()
                             },
                         },
                         ..Default::default()
                });
                let entry_vec: Vec<kvm_bindings::kvm_irq_routing_entry> = self.entries.values().cloned().collect();
                let irq_routing = kvm_bindings::IrqRouting::from_entries(&entry_vec).unwrap();
